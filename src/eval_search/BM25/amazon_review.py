...
if __name__ == '__main__':
    ...
    for i in tqdm(range(0, len(test_data), batch_size)):
        ...
        for query in queries:
            ...
            for topk in topks:
                ndcg[topk].append(ndcg_at_k(retrieved, targets[query], topk))
                recall[topk].append(recall_at_k(retrieved, [targets[query]], topk))
    
    
    # print mean
    for topk in topks:
        print(f'Recall@{topk}: {np.mean(recall[topk])}')
        print(f'NDCG@{topk}: {np.mean(ndcg[topk])}')