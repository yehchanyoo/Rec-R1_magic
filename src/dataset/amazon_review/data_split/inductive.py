...
if __name__ == '__main__':
    ...
    raw_data_dir = 'data/amazon_review/processed'
    output_dir = 'data/amazon_review/ori_split_files'
    
    for domain in domain_list:
        for split in ['train', 'valid', 'test']:
            input_file = os.path.join(raw_data_dir, domain, f'{domain}.{split}.inter')
            actual_split = 'val' if split == 'valid' else split
            output_file = os.path.join(output_dir, domain, f'{actual_split}.json')
            os.makedirs(os.path.dirname(output_file), exist_ok=True)

            convert(input_file, output_file, actual_split)