...
def load_rec_dataset(data_dir, setting_list, domain_name, meta_dir='data/amazon_review/processed/corpus', history_trunct_num=10):
    train_data_dict = {}
    val_data_dict = {}
    test_data_dict = {}
    ...
    for setting in setting_list:
        ...
        with open(os.path.join(data_dir, setting, domain_name, 'val.json'), 'r') as f:
    ...
    for setting in test_data_dict:
        train_data_dict[setting] = add_history_text(train_data_dict[setting], global_item2meta)
        val_data_dict[setting] = add_history_text(val_data_dict[setting], global_item2meta)
        test_data_dict[setting] = add_history_text(test_data_dict[setting], global_item2meta)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--domain_name', type=str, default='All_Beauty')
    ...
    train_data_dict, val_data_dict, test_data_dict = load_rec_dataset(file_dir, setting_list, args.domain_name)

    train_dataset_dict = {}
    val_dataset_dict = {}    
    test_dataset_dict = {}
    for setting in setting_list:
        train_data = train_data_dict[setting]
        train_dataset_dict[setting] = Dataset.from_list(train_data)

        val_data = val_data_dict[setting]
        val_dataset_dict[setting] = Dataset.from_list(val_data)

        test_data = test_data_dict[setting]
        test_dataset_dict[setting] = Dataset.from_list(test_data)
    ...
    for setting in setting_list:
        train_dataset = train_dataset_dict[setting]
        train_dataset_dict[setting] = train_dataset.map(function=make_map_fn('train', data_source_dict[setting]), with_indices=True)

        val_dataset = val_dataset_dict[setting]
        val_dataset_dict[setting] = val_dataset.map(function=make_map_fn('val', data_source_dict[setting]), with_indices=True)

        test_dataset = test_dataset_dict[setting]
        test_dataset_dict[setting] = test_dataset.map(function=make_map_fn('test', data_source_dict[setting]), with_indices=True)
    ...
    def convert_and_save(dataset, name, save_dir):
        # 1) Convert HuggingFace Dataset -> pandas DataFrame
        df = pd.DataFrame(dataset)

        # 2) Write out a valid Parquet file
    ...
    convert_and_save(train_dataset, "train", save_dir)
    convert_and_save(val_dataset,   "val",   save_dir)
    convert_and_save(test_dataset,  "test",  save_dir)
